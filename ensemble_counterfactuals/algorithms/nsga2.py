...
def ensemble_counter_nsga2(X,input,obj_class,test,discrete_variables=None,verbose=True,no_train=False):
    ...
    for i,model_name in enumerate(remaing_models):
        ...
        sol,score = _nsga2_ensemble(input,obj_class,cod,X,model_name,one_sol=False,discrete_variables=discrete_variables)
        if verbose:
            print(model_name)
            print(accuracy[i])
...
def _nsga2_ensemble(x_instance,y_desired,codification,X,model_name,pop_size=20,n_gen=20,period=10,one_sol=True,discrete_variables=None,graphic=False):
    ...
    algo = NSGA2(pop_size=pop_size,save_history=True)
